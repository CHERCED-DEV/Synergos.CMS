...
services:
  ...
  cms:
    ...
    environment:
      ...
      # WebRouting y NO Global — la clave vive ahi y el binder de .NET descarta en
      # SILENCIO lo que no mapea. Escrita en Global, la URL publica no llegaba nunca:
      # quedaba la del perfil (http://localhost:8080/), asi que toda URL absoluta que
      # Umbraco construye en produccion —los links del backoffice, los de las
      # notificaciones— apuntaba a localhost, y el KeepAliveJob escribia «No
      # umbracoApplicationUrl for service (yet), skip» cada sesenta segundos. Es el #138
      # en el artefacto que su gate no leia; hoy hay gate (#159).
      ...
      # Y el ARTEFACTO, que NO viaja: el secreto con el que se firma el QR de la entrada.
      # Vacio no es "no firmar" — el host genera una llave y la guarda cifrada en
      # cms-dpkeys — pero entonces NO se puede rotar, y ese es el unico camino para
      # compartirla entre instalaciones. Hasta el #154 esta variable no existia, asi que
      # ponerla en el .env del servidor no llegaba al contenedor: una clave que nadie lee.
      Synergos__Eventos__Ticket__SigningSecret: \${SYNERGOS_EVENTOS_TICKET_SECRET:-}

      # Y contra quien se reserva un viaje (HU #36 + #40). Cruza las DOS vias: la
      # reserva de hotel y el carrito multi-producto, que pide confirmacion
      # PARCIAL y ordena la devolucion de lo que no se cumplio. Encenderlo exige
      # ademas cargar el precio en Api.Pricing y el recurso en Api.Booking por
      # cada oferta que se venda — ver .env.example.
      ...
      # Quien custodia la llave que identifica un diploma (hallazgo #45). Lo que se
      # gana es la CUSTODIA: rotar y RETIRAR una llave, que la local no sabe hacer.
      # El firmante local NO se descarta — queda verificando los ids emitidos antes
      # del cableado, o cada QR ya impreso dejaria de valer.
      #
      # Encenderlo exige crear la llave en Api.Signing (POST /v1/keys con
      # purpose=academy.certificate) — ver .env.example.
      Synergos__Academy__Mode: \${SYNERGOS_ACADEMY_MODE:-Stub}
      Synergos__Academy__BaseUrl: "http://api-signing:8080"
      ...
      # Y su gemelo del eje 3, por la misma razon y con un precio mas alto: CLAUDE.md §11
      # dice que este secreto es lo UNICO que el respaldo no puede regenerar —sin el, los
      # diplomas ya emitidos dejan de verificar— y que "va donde va la identidad", o sea en
      # el .env del servidor. No llegaba al contenedor (#154).
      Synergos__Academy__CertificateSigningSecret: \${SYNERGOS_ACADEMY_CERT_SECRET:-}
      ...
      # Contra que se pone un acto administrativo en conocimiento (HU #62). Va por
      # SU PROPIO interruptor: notificar es Api.Messaging y decidir es Api.Workflow,
      # y un despliegue puede querer una sin la otra. Con la capacidad caida no se
      # notifica ni se abre —no se da por notificado en silencio— pero la bandeja
      # del ciudadano se sigue viendo, porque se lee de este lado.
      Synergos__Gob__Notifications__Mode: \${SYNERGOS_GOB_NOTIFICATIONS_MODE:-Local}
      Synergos__Gob__Notifications__BaseUrl: "http://api-messaging:8080"
      Synergos__Gob__Notifications__ApiKey: \${SYNERGOS_API_KEY}
      Synergos__Gob__Notifications__EntityKind: \${SYNERGOS_GOB_ENTITY_KIND:-gov.entidad}
      Synergos__Gob__Notifications__EntityId: \${SYNERGOS_GOB_ENTITY_ID:-ventanilla}

      # Si el asiento de la bitacora ademas sale de esta maquina (HU #15). El
      # JSONL de App_Data NO se sustituye: envuelve, y sigue siendo el modelo de
      # lectura — con Api.Audit caida el administrador SIGUE viendo que paso, y
      # lo que se para es que el asiento salga. Si el reenvio no llega, queda
      # escrito localmente que no llego. El correo de quien actuo no viaja:
      # viaja un seudonimo.
      Synergos__Audit__Mode: \${SYNERGOS_AUDIT_MODE:-Local}
      Synergos__Audit__BaseUrl: "http://api-audit:8080"
      Synergos__Audit__ApiKey: \${SYNERGOS_API_KEY}
      Synergos__Audit__ActorKind: \${SYNERGOS_AUDIT_ACTOR_KIND:-cms.actor}
      Synergos__Audit__TargetKind: \${SYNERGOS_AUDIT_TARGET_KIND:-cms.recurso}
