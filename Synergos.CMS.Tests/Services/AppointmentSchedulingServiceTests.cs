...
public sealed class AppointmentSchedulingServiceTests
{
    ...
    public void EmptyExisting_NoConflict() =>
        Assert.False(AppointmentSchedulingService.HasConflict(System.Array.Empty<AppointmentSlot>(), D, D.AddHours(1), 0));

    [Fact]
    public void Adjacent_NoOverlap_NoConflict()
    {
        var existing = new[] { Slot(D, D.AddHours(1)) };
        Assert.False(AppointmentSchedulingService.HasConflict(existing, D.AddHours(1), D.AddHours(2), 0));   // 10-11 vs 9-10
    }
    ...
    {
        var existing = new[] { Slot(D, D.AddHours(1)) };
        Assert.True(AppointmentSchedulingService.HasConflict(existing, D.AddMinutes(30), D.AddMinutes(90), 0));   // 30 min solape
    }
    ...
    public void Overlap_WithinTolerance_NoConflict()
    {
        ...
        Assert.False(AppointmentSchedulingService.HasConflict(existing, D.AddMinutes(50), D.AddMinutes(110), 15));
    }
    ...
    public void Overlap_BeyondTolerance_Conflict()
    {
        ...
        Assert.True(AppointmentSchedulingService.HasConflict(existing, D.AddMinutes(40), D.AddMinutes(100), 15));
    }
    ...
    public void CancelledAppointment_Ignored()
    {
        ...
        Assert.False(AppointmentSchedulingService.HasConflict(existing, D.AddMinutes(30), D.AddMinutes(90), 0));
    }
