...
public sealed class WebhookCommentModerationNotifier : ICommentModerationNotifierChannel
{
    ...
    public async Task NotifyPendingAsync(Comment comment, CancellationToken cancellationToken)
    {
        ...
        if (signed is { } s)
        {
            request.Headers.TryAddWithoutValidation(WebhookSigner.TimestampHeaderName, s.Timestamp);
            request.Headers.TryAddWithoutValidation(WebhookSigner.SignatureHeaderName, s.Signature);
        }
