...
| `syn-search-analytics/{yyyy-MM-dd}.jsonl` | **Declarado pero nunca escrito**: `ISearchAnalyticsStore` real es `InMemorySearchAnalyticsStore`, puramente `ConcurrentDictionary`, sin persistencia a disco | Ninguno — `SearchAnalyticsRetentionPolicy` barre un directorio que ningún writer puebla | 90 días (sobre un directorio vacío) | **No** — se pierde en cada reinicio |
...
1. **`ISearchAnalyticsStore` no persiste, pero su retención sí existe** — trabajo muerto: `Synergos.CMS.Web/Services/Retention/SearchAnalyticsRetentionPolicy.cs:14` barre `App_Data/syn-search-analytics/`, directorio que `Synergos.CMS.Web/Services/InMemorySearchAnalyticsStore.cs` (registrado en `SeamComposer.cs:510`) nunca escribe. Efecto real: las estadísticas de búsqueda del admin se pierden en cada reinicio del proceso, contradiciendo la promesa implícita de "top queries 7d" que aparece en `AdminController.Index` (`AdminController.cs:117-118`).
...
| Search analytics (in-memory, sin persistencia) | `Synergos.CMS.Web/Services/InMemorySearchAnalyticsStore.cs` |
