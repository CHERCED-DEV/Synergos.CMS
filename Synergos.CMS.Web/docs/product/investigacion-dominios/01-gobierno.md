...
| **Notificación electrónica**: solo si el administrado la aceptó, y surte efecto desde que accede — **la administración debe certificar fecha y hora del acceso** | Colombia — Ley 2080/2021 | No basta el log de envío de `Api.Notifications`. Hace falta **acuse de acceso certificado** (quién abrió, cuándo, con evidencia sellada). `Api.Messaging` tenía `POST /v1/messages/{id}/read`, que era el germen correcto. **Hecho en la HU #13**: hoy es `POST /v1/messages/{id}/acknowledge` y guarda quién, cuándo y con qué se afirmó la identidad. | VERIFICADO — [Ley 2080 de 2021](https://www.funcionpublica.gov.co/eva/gestornormativo/norma.php?i=156590) |
...
- **`Synergos.Api.Messaging`** — hilos con participantes cerrados, rechazo a quien no es participante, y `POST /v1/messages/{id}/acknowledge` (HU #13). Es la correspondencia del expediente.
