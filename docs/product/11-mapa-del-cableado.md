...
| `StubHotelBookingService` | `Bff.Viajes` `POST /v1/trips` | **Orquestador.** Apartar, cobrar y confirmar pueden fallar a la mitad. Sólo la vía hotel. El carrito multi-producto ya lleva periodo (#40) y sigue sin cablear por **otro** motivo: falta verificar con procesos vivos que tres ítems en fechas distintas apartan tres ventanas distintas | **HU #36** |
