...
| `StubCertificateService` | `Api.Signing` — **pero NO `/v1/signatures`** | **Capacidad.** El motivo sigue en pie: el HMAC local (ADR 0124) guarda su llave y **no sabe retirarla**, y la capacidad sí. Lo que no encaja es el endpoint: su token **vence** (≤365 d), **no es determinista** (el vencimiento va dentro de lo firmado) y **publica el payload** —o sea el alumno— dentro del id que se imprime. Hace falta un modo de firma determinista y opaco. Ver #45 | **bloqueado por #45** |
