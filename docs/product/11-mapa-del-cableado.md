...
| `StubCaseWorkflowService` | `Api.Workflow` `/v1/instances/{id}/fire` | **Capacidad.** Una transición de estado es un paso que dice NO solo (`illegal_transition`). Hoy la tabla de transiciones vive en C# dentro del stub, que es justo lo que `Api.Workflow` existe para no repetir por dominio | pendiente |
| `StubCertificateService` | `Api.Signing` `/v1/signatures` | **Capacidad.** Firmar y verificar es un paso. Hoy el id lo firma un HMAC local con llave propia (ADR 0124); `Api.Signing` es dueña de las llaves y de su retiro, que es lo que el local no sabe hacer | pendiente |
