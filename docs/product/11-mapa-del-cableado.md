...
| `StubHotelBookingService` | `Bff.Viajes` `POST /v1/trips` | **Orquestador.** Apartar, cobrar y confirmar pueden fallar a la mitad. Sólo la vía hotel. El carrito multi-producto ya lleva periodo, modo de fallo elegible y motor aparte (#40, rebanadas 1 y 2); falta la 3 — **el segundo motor**, el que habla con el orquestador, y verificar con procesos vivos que tres ítems en fechas distintas apartan tres ventanas distintas | **HU #36** |
